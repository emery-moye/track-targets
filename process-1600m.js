...
function processFile() {
    ...
    if (currentConference && TARGET_CONFERENCES.includes(currentConference) && 
      ...
      if (!has1600m) {
        ...
        if (targetMatch && recruitMatch) {
          ...
          schoolsProcessed.push(schoolKey);
          console.log(`✓ ${schoolKey}: ${target1600} / ${recruit1600}`);
        }
      }
    }
  }
  ...
  console.log('\n' + '='.repeat(70));
  console.log(`Processed ${schoolsProcessed.length} total programs`);
  console.log('='.repeat(70));
}
...
try {
  ...
  console.log('\nFormulas used:');
  console.log('  Men: 1600m = 1500m × 1.096');
  console.log('  Women: 1600m = 1500m × 1.138');
} catch (error) {
  console.error('\n❌ Error:', error.message);
