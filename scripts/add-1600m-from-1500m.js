...
function processFile() {
    ...
    if (currentConference && TARGET_CONFERENCES.includes(currentConference) && 
      ...
      if (!has1600m) {
        ...
        if (targetMatch && recruitMatch) {
          ...
          console.log(`Added 1600m for ${currentConference} (${currentGender}): ${target1600}`);
        }
      }
    }
    
    // Track school processing
    if (line.includes('name:') && currentConference && TARGET_CONFERENCES.includes(currentConference)) {
      processedSchools++;
    }
  }
  ...
  console.log(`Schools processed: ${processedSchools}`);
  console.log('\nBreakdown by conference:');
  Object.entries(conferenceStats).forEach(([conf, count]) => {
    console.log(`  ${conf}: ${count} standards added`);
  });
}
