...
// Target conferences
const TARGET_CONFERENCES = ['MEAC', 'Big Sky', 'Big East'];
...
function processFile() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inTargetConference && currentGender && line.includes('"1500m":')) {
      ...
      // Check if next line is Mile or 1600m (skip if 1600m already exists)
      if (i + 1 < lines.length && lines[i + 1].includes('"Mile":')) {
        ...
        if (targetMatch && recruitMatch) {
          ...
          // All three conferences have walkon standards
          if (walkonMatch) {
