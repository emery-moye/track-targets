    ...
    if (!lookback.includes('"300m Hurdles"')) {
      ...
      if (targetMatch && recruitMatch) {
        ...
        if (!processedSchools.has(schoolKey)) {
          processedSchools.set(schoolKey, currentConference || 'Unknown');
          console.log(`✓ ${currentSchool} (${currentGender}) - ${currentConference || 'Unknown'}`);
        }
      }
    }
  }
...
if (addedCount === 0) {
  ...
  for (const [school, conference] of processedSchools) {
    conferenceStats.set(conference, (conferenceStats.get(conference) || 0) + 1);
  }
  
  console.log('\n📍 By Conference:');
  for (const [conference, count] of conferenceStats) {
